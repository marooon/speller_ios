...
- (void)setHidden:(BOOL)hidden animated:(BOOL)animated
{
	...
	{		
		...
		animation.fromValue = [NSNumber numberWithFloat:hidden ? 1 : 0];
		animation.toValue = [NSNumber numberWithInt:hidden ? 0 : 1];
		[self.layer addAnimation:animation forKey:@"hidden"];
		
//		[UIView beginAnimations:hidden ? @"hidden" : @"shown" context:NULL];
...
/*
#pragma mark -
#pragma mark UIAnimationgDelegate
- (void)animationWillStart:(NSString *)animationID context:(void *)context
{
	if([animationID isEqual:@"hidden"])
	{
		
	}
	else if([animationID isEqual:@"shown"])
	{
		self.hidden = NO;
		self.alpha = 0.0;
	}
...
+ (UIColor*)RandomColor
{
    int r = rand() % 255;
    float red = 0.9;
    float green = (float)r/(float)255;
    float blue = (float)r/(float)255;
